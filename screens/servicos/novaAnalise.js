...
const CadastroAnalise = ({ route, navigation }) => {
  useLogScreen('CadastroAnalise');
  const { id = null, analise_id, status = 1, usuario_id, classificacao = '', tipo = '', status_user } = route.params || {};
  const [usuario, setUsuario] = useState({ status_user });

  const [imovel, setImovel] = useState({ id, classificacao, tipo, usuario_id });
  const [loading, setLoading] = useState(false);
  console.log("analise_id:", analise_id)
  ...
  console.log("status novaanalise.js", status)
  const steps = [
    { label: 'Dados do proprietário', status: 1, view: 'OneCadastroAnalise' },
    { label: 'Dados do imóvel', status: 2, view: 'TwoCadastroAnalise' },
